...
public class GetQueuesTests : RetryQueueDataProviderTestsTemplate
{
    ...
    public async Task
        GetQueuesAsync_DifferentSearchGroupKeyDifferentQueueStatusDifferentItemStatus_ReturnOnlyRequestedQueuesAndItems(
    ...
    [InlineData(RepositoryType.SqlServer)]
    [InlineData(RepositoryType.Postgres)]
    public async Task GetQueuesAsync_ExistingQueuesActiveButDifferentSearchGroupKey_DontReturnQueues(
        RepositoryType repositoryType)
    {
        // Arrange
        var repository = GetRepository(repositoryType);
    ...
    [InlineData(RepositoryType.SqlServer)]
    [InlineData(RepositoryType.Postgres)]
    public async Task GetQueuesAsync_ExistingQueueWithDifferentItemStatus_ReturnQueueWithoutItems(
        RepositoryType repositoryType)
    {
        // Arrange
        var repository = GetRepository(repositoryType);
    ...
    [InlineData(RepositoryType.SqlServer)]
    [InlineData(RepositoryType.Postgres)]
    public async Task GetQueuesAsync_ExistingQueueWithDifferentQueueStatus_DontReturnQueues(
        RepositoryType repositoryType)
    {
        // Arrange
        var repository = GetRepository(repositoryType);
    ...
    [InlineData(RepositoryType.SqlServer)]
    [InlineData(RepositoryType.Postgres)]
    public async Task GetQueuesAsync_ExistingQueueWithDistinctItemStatus_ReturnsQueueWithFilteredItems(
        RepositoryType repositoryType)
    {
        // Arrange
        var repository = GetRepository(repositoryType);
    ...
    [InlineData(RepositoryType.SqlServer)]
    [InlineData(RepositoryType.Postgres)]
    public async Task GetQueuesAsync_ItemsWithDifferentModifiedDates_ReturnQueueWithItemsandQueueWithNoItem(
        RepositoryType repositoryType)
    {
        // Arrange
        var repository = GetRepository(repositoryType);
    ...
    [InlineData(RepositoryType.SqlServer)]
    [InlineData(RepositoryType.Postgres)]
    public async Task GetQueuesAsync_WithSeverityLevel_ReturnsOnlyItemsWithCorrespondingLevel(
        RepositoryType repositoryType)
    {
        // Arrange
        var repository = GetRepository(repositoryType);

        var input = GetQueuesInputWithSeverites(new[] { SeverityLevel.High, SeverityLevel.Medium });
        ...
        actualQueues.SelectMany(o => o.Items.Where(i => i.SeverityLevel == SeverityLevel.Medium)).Count().Should()
            .Be(1);
    ...
    [InlineData(RepositoryType.SqlServer)]
    [InlineData(RepositoryType.Postgres)]
    public async Task GetQueuesAsync_WithStuckStatusFilter_ReturnsItemsByStatusAndStuckItems(
        RepositoryType repositoryType)
    {
        // Arrange
        var repository = GetRepository(repositoryType);
    ...
    private GetQueuesInput GetQueuesWithStuckStatusInput(string searchGroupKey)
    {
        return new GetQueuesInput(
            ...
            GetQueuesSortOption.ByLastExecutionAscending,
            100,
            new StuckStatusFilter(RetryQueueItemStatus.InRetry, new TimeSpan(0, 3, 0)))
        {
            SearchGroupKey = searchGroupKey
        };
    }

...
public class GetItemsResponseDtoAdapterTests
{
    ...
    public void GetItemsResponseDtoAdapter_Adapt_Success()
    {
        ...
        var queueA = new RetryQueue(Guid.NewGuid(), "searchGroupKeyA", "queueGroupKeyA", DateTime.UtcNow,
            DateTime.UtcNow, RetryQueueStatus.Active, itemsA);
        var queueB = new RetryQueue(Guid.NewGuid(), "searchGroupKeyB", "queueGroupKeyB", DateTime.UtcNow,
            DateTime.UtcNow, RetryQueueStatus.Done, itemsB);
        ...
        responseDto.QueueItems.Should()
            .BeEquivalentTo(expectedRetryQueueItems, options => options.ExcludingMissingMembers());
    }

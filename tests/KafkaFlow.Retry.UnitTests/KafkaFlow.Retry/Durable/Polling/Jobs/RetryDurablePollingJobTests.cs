...
public class RetryDurablePollingJobTests
{
    ...
    private static readonly RetryDurablePollingDefinition s_retryDurablePollingDefinition =
        new(true, "0 0 14-6 ? * FRI-MON", 1, 1);

    private readonly IJob _job = new RetryDurablePollingJob();
    private readonly Mock<IJobExecutionContext> _jobExecutionContext = new();
    private readonly Mock<ILogHandler> _logHandler = new();
    private readonly Mock<IMessageAdapter> _messageAdapter = new();
    private readonly Mock<IMessageHeadersAdapter> _messageHeadersAdapter = new();
    private readonly Mock<IMessageProducer> _messageProducer = new();
    private readonly Mock<IJobDetail> _mockIJobDetail = new();
    private readonly Mock<ITrigger> _mockITrigger = new();
    private readonly Mock<IRetryDurableQueueRepository> _retryDurableQueueRepository = new();
    private readonly Mock<IUtf8Encoder> _utf8Encoder = new();
    ...
    public async Task RetryDurablePollingJob_Execute_ProduceMessageFailed_LogError()
    {
        ...
        _retryDurableQueueRepository
            ...
            .ReturnsAsync(new List<RetryQueue>
            {
                new(
                    ...
                    {
                        new(Guid.NewGuid(), 1, DateTime.UtcNow, 0, null, null, RetryQueueItemStatus.Waiting,
        ...
        //Assert
        _logHandler.Verify(d => d.Error(It.IsAny<string>(), It.IsAny<Exception>(), It.IsAny<object>()),
            Times.Exactly(2));
        _retryDurableQueueRepository.Verify(d => d.GetRetryQueuesAsync(It.IsAny<GetQueuesInput>()), Times.Once);
        _retryDurableQueueRepository.Verify(d => d.UpdateItemAsync(It.IsAny<UpdateItemStatusInput>()),
            Times.Exactly(2));
        _messageHeadersAdapter.Verify(d => d.AdaptMessageHeadersFromRepository(It.IsAny<IList<MessageHeader>>()),
            Times.Once);
    ...
    public async Task RetryDurablePollingJob_Execute_RetryDurableQueueRepositoryFailed_LogError()
    {
        ...
        //Assert
        _messageProducer.Verify(
            d => d.ProduceAsync(It.IsAny<byte[]>(), It.IsAny<byte[]>(), It.IsAny<IMessageHeaders>(), It.IsAny<int?>()),
            Times.Never);
        _logHandler.Verify(d => d.Error(It.IsAny<string>(), It.IsAny<RetryDurableException>(), It.IsAny<object>()),
            Times.Once);
        _retryDurableQueueRepository.Verify(d => d.GetRetryQueuesAsync(It.IsAny<GetQueuesInput>()), Times.Once);

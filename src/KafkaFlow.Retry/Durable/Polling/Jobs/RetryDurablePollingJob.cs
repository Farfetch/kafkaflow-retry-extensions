...
internal class RetryDurablePollingJob : IJob
{
    ...
    public async Task Execute(IJobExecutionContext context)
    {
        ...
        var retryDurablePollingDefinition =
            jobDataMap.GetValidValue<RetryDurablePollingDefinition>(PollingJobConstants.RetryDurablePollingDefinition,
                nameof(RetryDurablePollingJob));
        var schedulerId =
            jobDataMap.GetValidStringValue(PollingJobConstants.SchedulerId, nameof(RetryDurablePollingJob));
        var retryDurableQueueRepository =
            jobDataMap.GetValidValue<IRetryDurableQueueRepository>(PollingJobConstants.RetryDurableQueueRepository,
                nameof(RetryDurablePollingJob));
        var logHandler =
            jobDataMap.GetValidValue<ILogHandler>(PollingJobConstants.LogHandler, nameof(RetryDurablePollingJob));
        var messageHeadersAdapter =
            jobDataMap.GetValidValue<IMessageHeadersAdapter>(PollingJobConstants.MessageHeadersAdapter,
                nameof(RetryDurablePollingJob));
        var utf8Encoder =
            jobDataMap.GetValidValue<IUtf8Encoder>(PollingJobConstants.Utf8Encoder, nameof(RetryDurablePollingJob));
        var retryDurableProducer =
            jobDataMap.GetValidValue<IMessageProducer>(PollingJobConstants.RetryDurableMessageProducer,
                nameof(RetryDurablePollingJob));
            ...
            foreach (var queue in activeQueues)
            {
                if (!queue.Items.Any())
                {
                    logHandler.Verbose(
                        ...
                        new
                        {
                            QueueId = queue.Id, queue.QueueGroupKey
                        }
                    );

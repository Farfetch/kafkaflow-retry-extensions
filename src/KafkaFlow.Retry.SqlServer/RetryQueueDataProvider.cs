...
internal sealed class RetryQueueDataProvider : IRetryDurableQueueRepositoryProvider
{
    ...
    public async Task<CheckQueueResult> CheckQueueAsync(CheckQueueInput input)
    {
        ...
        {
            var exists = await _retryQueueRepository.ExistsActiveAsync(dbConnection, input.QueueGroupKey)
                .ConfigureAwait(false);

            return new CheckQueueResult(
                exists ? CheckQueueResultStatus.Exists : CheckQueueResultStatus.DoesNotExist);
        }
    }
    ...
    public async Task<QueueNewestItemsResult> CheckQueueNewestItemsAsync(QueueNewestItemsInput input)
    {
        ...
        using (var dbConnection = _connectionProvider.Create(_sqlServerDbSettings))
        {
            var itemsDbo = await _retryQueueItemRepository.GetNewestItemsAsync(dbConnection, input.QueueId, input.Sort)
                .ConfigureAwait(false);
    ...
    public async Task<QueuePendingItemsResult> CheckQueuePendingItemsAsync(QueuePendingItemsInput input)
    {
        ...
        using (var dbConnection = _connectionProvider.Create(_sqlServerDbSettings))
        {
            var itemsDbo = await _retryQueueItemRepository.GetPendingItemsAsync(dbConnection, input.QueueId, input.Sort)
                .ConfigureAwait(false);
    ...
    public async Task<GetQueuesResult> GetQueuesAsync(GetQueuesInput input)
    {
        ...
        using (var dbConnection = _connectionProvider.Create(_sqlServerDbSettings))
        {
            ...
            dboWrapper.MessagesDbos = await _retryQueueItemMessageRepository
                .GetMessagesOrderedAsync(dbConnection, dboWrapper.ItemsDbos).ConfigureAwait(false);
            dboWrapper.HeadersDbos = await _retryQueueItemMessageHeaderRepository
                .GetOrderedAsync(dbConnection, dboWrapper.MessagesDbos).ConfigureAwait(false);
        }
    ...
    public async Task<SaveToQueueResult> SaveToQueueAsync(SaveToQueueInput input)
    {
        ...
        {
            var retryQueueDbo = await _retryQueueRepository.GetQueueAsync(dbConnection, input.QueueGroupKey)
                .ConfigureAwait(false);
    ...
    public async Task<UpdateItemResult> UpdateItemStatusAsync(UpdateItemStatusInput input)
    {
        ...
        using (var dbConnection = _connectionProvider.Create(_sqlServerDbSettings))
        {
            ...
            return new UpdateItemResult(
                input.ItemId,
                totalItemsUpdated == 0 ? UpdateItemResultStatus.ItemNotFound : UpdateItemResultStatus.Updated);
        }
    }
    ...
    private async Task AddItemAsync(IDbConnection dbConnection, SaveToQueueInput input, long retryQueueId,
        Guid retryQueueDomainId)
    {
        var retryQueueItemDbo = _retryQueueItemDboFactory.Create(input, retryQueueId, retryQueueDomainId);

        var retryQueueItemId =
            await _retryQueueItemRepository.AddAsync(dbConnection, retryQueueItemDbo).ConfigureAwait(false);
        ...
        var retryQueueHeadersDbo =
            _retryQueueItemMessageHeaderDboFactory.Create(input.Message.Headers, retryQueueItemId);
        await _retryQueueItemMessageHeaderRepository.AddAsync(dbConnection, retryQueueHeadersDbo).ConfigureAwait(false);
    }

    private async Task AddItemIntoAnExistingQueueAsync(IDbConnection dbConnection, SaveToQueueInput input,
        RetryQueueDbo retryQueueDbo)
    {
        ...
        if (retryQueueDbo.Status == RetryQueueStatus.Done)
        {
            ...
            await _retryQueueRepository.UpdateStatusAsync(dbConnection, retryQueueDbo.IdDomain, RetryQueueStatus.Active)
                .ConfigureAwait(false);
        }
    }
    ...
    private async Task<UpdateQueueResultStatus> TryUpdateQueueToDoneAsync(IDbConnectionWithinTransaction dbConnection,
        Guid queueId)
    {
        var anyItemStillActive = await _retryQueueItemRepository.AnyItemStillActiveAsync(dbConnection, queueId)
            .ConfigureAwait(false);

        if (!anyItemStillActive)
        {
            var queueRowsAffected = await _retryQueueRepository
                .UpdateStatusAsync(dbConnection, queueId, RetryQueueStatus.Done).ConfigureAwait(false);
    ...
    private async Task<UpdateItemResult> UpdateItemAndQueueStatusAsync(UpdateItemStatusInput input)
    {
        ...
        using (var dbConnection = _connectionProvider.CreateWithinTransaction(_sqlServerDbSettings))
        {
            ...
            var updateQueueResultStatus =
                await TryUpdateQueueToDoneAsync(dbConnection, item.DomainRetryQueueId).ConfigureAwait(false);
    ...
    private async Task<UpdateItemResult> UpdateItemAndTryUpdateQueueToDoneAsync(UpdateItemExecutionInfoInput input)
    {
        ...
        {
            //update item
            var itemRowsAffected = await _retryQueueItemRepository.UpdateAsync(dbConnection, input.ItemId, input.Status,
                input.AttemptCount, input.LastExecution, input.Description).ConfigureAwait(false);
            ...
            var updateQueueResultStatus =
                await UpdateQueueLastExecutionAndTryUpdateQueueToDoneAsync(dbConnection, input.QueueId,
                    input.LastExecution).ConfigureAwait(false);
    ...
    private async Task<UpdateQueueResult> UpdateQueueAndAllItemsAsync(UpdateItemsInQueueInput input)
    {
        using (var dbConnection = _connectionProvider.CreateWithinTransaction(_sqlServerDbSettings))
        {
            var queue = await _retryQueueRepository.GetQueueAsync(dbConnection, input.QueueGroupKey)
                .ConfigureAwait(false);
            ...
            {
                return new UpdateQueueResult(input.QueueGroupKey, UpdateQueueResultStatus.UpdateIsNotAllowed,
                    queue.Status);
            }

            if (queue.Status != RetryQueueStatus.Active)
            {
                return new UpdateQueueResult(input.QueueGroupKey, UpdateQueueResultStatus.QueueIsNotActive,
                    queue.Status);
            }

            var items = await _retryQueueItemRepository
                .GetItemsOrderedAsync(dbConnection, new[] { queue.IdDomain }, new[] { RetryQueueItemStatus.Waiting })
                .ConfigureAwait(false);
            ...
            foreach (var item in items)
            {
                var updateItemResult =
                    await UpdateItemStatusAsync(new UpdateItemStatusInput(item.IdDomain, input.ItemStatus))
                        .ConfigureAwait(false);

                if (updateItemResult.Status != UpdateItemResultStatus.Updated)
                {
                    return new UpdateQueueResult(input.QueueGroupKey, UpdateQueueResultStatus.FailedToUpdateItems,
                        queue.Status);
                }
            }

            var updateQueueResultStatus =
                await TryUpdateQueueToDoneAsync(dbConnection, queue.IdDomain).ConfigureAwait(false);
    ...
    private async Task<UpdateQueueResultStatus> UpdateQueueLastExecutionAndTryUpdateQueueToDoneAsync(
        IDbConnectionWithinTransaction dbConnection, Guid queueId, DateTime lastExecution)
    {
        ...
        if (anyItemStillActive)
        {
            ...
            var queueLastExecutionRowsAffected = await _retryQueueRepository
                .UpdateLastExecutionAsync(dbConnection, queueId, lastExecution).ConfigureAwait(false);
        ...
        else
        {
            ...
            var queueRowsAffected = await _retryQueueRepository
                .UpdateAsync(dbConnection, queueId, RetryQueueStatus.Done, lastExecution).ConfigureAwait(false);
